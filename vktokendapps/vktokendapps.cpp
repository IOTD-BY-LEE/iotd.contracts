...
public:
  static random_gen& get_instance(name account) {
    instance.seed = now() + account.value;
    return instance;
  }
...
public:
  ...
  ACTION reward(name account) {
    ...
    if(itr == users.end()){
      itr = users.emplace(_self, [&](auto& user){
    ...
    if(!is_today(itr->last_reward_time.sec_since_epoch ())){
      ...
      users.modify(itr, _self, [&](auto& user){
        user.last_reward_time = time_point_sec(now()); 
  ...
  ACTION comment(name author, const std::string& content, uint64_t post_id,
    ...
    comments.emplace(author, [&](auto& comment){
      ...
      comment.time = time_point_sec(now()); 
...
private:
  ...
  uint32_t get_reward(name account){
    uint32_t seconds = now() - GENESIS_TIME;
    if(seconds <= 30*86400){
    ...
    if(liker != itr->author){
      ...
      if(!istoday || (istoday && liker_itr->like_times < 10)){
        ...
        users.modify(liker_itr, same_payer, [&](auto& user){
          user.balance += tokens;
          user.last_like_time = time_point_sec(now()); 
  ...
  void issue_token(asset quantity){
    action(
      ...
      "eosio.token"_n,
      "issue"_n,
      std::make_tuple(_self, quantity, std::string("reward issue token"))
    ).send();
  }
  ...
  void transfer_token(name to, asset quantity){
    action(
      ...
      "eosio.token"_n,
      "transfer"_n,
      std::make_tuple(_self, to, quantity, std::string("reward transfer token"))
    ).send();
  }
...
EOSIO_DISPATCH( vktokendapps, (reward) )
