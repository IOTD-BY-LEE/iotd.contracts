...
void trans::deposit(name from, vector<account_record_content> content)
{
    ...
    {
       eosio_assert(symbol("VKT",4).code() == record_itr->assets.symbol.code(), "Deposit asset symbol error");
    ...
    for (auto record_itr = acc_record.begin(); record_itr != acc_record.end(); ++record_itr)
    {
        ...
        else
        {
            list.modify(itr, same_payer, [&](auto &tmp_record) {
                ...
                eosio_assert(symbol("VKT",4).code() == itr->assets.symbol.code(), "Deposit asset symbol error");
                eosio_assert(record_itr->assets.symbol.code() == itr->assets.symbol.code(), "invalid bool_positive");
                tmp_record.account_time = current_time();
